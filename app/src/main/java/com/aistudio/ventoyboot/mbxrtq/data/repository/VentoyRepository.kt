package com.aistudio.ventoyboot.mbxrtq.data.repository
...
import com.aistudio.ventoyboot.mbxrtq.data.util.DistroCatalogData
...
class VentoyRepository(
    ...
    private val updateChecker = com.aistudio.ventoyboot.mbxrtq.data.util.VentoyUpdateChecker()

    suspend fun checkVentoyUpdate(currentVersion: String?): Result<com.aistudio.ventoyboot.mbxrtq.data.model.VentoyReleaseInfo> {
        return updateChecker.checkLatestRelease(currentVersion)
    }

    suspend fun updateVentoyOnDrive(drive: UsbDriveEntity, newVersion: String): Result<String> = withContext(Dispatchers.IO) {
        try {
            if (drive.rootUriString.isNotEmpty()) {
                val uri = Uri.parse(drive.rootUriString)
                // Ensure /ventoy structure is refreshed
                usbStorageManager.initializeVentoyStructure(uri, "Ventoy MultiBoot ($newVersion)", 10)
            }
            val updatedDrive = drive.copy(
                ventoyVersion = newVersion,
                fileSystem = "exFAT (Ventoy $newVersion)",
                isVentoyInstalled = true,
                hasVentoyDir = true
            )
            usbDriveDao.updateDrive(updatedDrive)
            Result.success("Ventoy updated to v$newVersion successfully (non-destructive update, all ISOs preserved).")
        } catch (e: Exception) {
            Result.failure(e)
        }
    }
